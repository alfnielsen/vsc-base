...
const mapImports = (content: string, _imports: ts.ImportDeclaration[], options: SortImportsOptions) => {
   ...
   const imports: Imports = _imports.map((node, index) => {
      ...
      if (importClause) {
         ...
         if (importClause.namedBindings && ts.isNamedImports(importClause.namedBindings)) {
            specifiers = importClause.namedBindings.elements.map(e => ({
               ...
               node: e,
               name: e.name.getText(),
               isUsed: isUsed(content, e.name.getText(), e.pos, e.end)
            }))
            if (options.removeUnusedImports) {
               const exclude = options.removeUnusedImportsExcludeList || []
      ...
      const used = isUsed(content, alias, node.pos, node.end)
      if (options.removeUnusedImports) {
         const exclude = options.removeUnusedImportsExcludeList || []
...
export const isUsed = (source: string, name: string, pos: number, end: number): boolean => {
