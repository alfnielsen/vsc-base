...
enum ReplacerType {
  FilesAndFolderName = "FilesAndFolderName",
      ...
      body: `
            ...
            <div class='settings'>
               ...
               ${renderCheckbox(
                 "useReg",
                 state.useReg,
                 "Use RegExp<sup>1</sup>"
               )}<br/>
               <div class='hrsubline'>&nbsp;</div>
               Add index ${renderCheckbox(
                 "addIndexWhenExist",
                 state.addIndexWhenExist,
                 " when coping to an exiting name"
               )}<br/>
               ...
               ${renderCheckbox(
                 "replaceIndexForFolders",
                 state.replaceIndexForFolders,
                 " in folder name"
               )}
               <br/>
               Reset the $index:
               ${renderCheckbox(
                 "resetIndexForFolders",
                 state.resetIndexForFolders,
                 " for folders"
               )}
               ${renderCheckbox(
                 "resetIndexForEachFolder",
                 state.resetIndexForEachFolder,
                 " for each new folder (File names)"
               )}
               <br/>
               ...
               ${renderCheckbox(
                 "setIndexAtEndOfFolder",
                 state.setIndexAtEndOfFolder,
                 "For folder name"
               )}
            </div>
            <h3>Replace:</h3>
            ...
            <div id="replacer">
               Replace <input type="text" onKeyup="sendCommand('setReplacerProp',{prop:'from', value:this.value})" 
                  ...
                  <select onChange="sendCommand('setReplacerProp',{prop:'appliesTo', value:this.value})">
                     <option value="${
                       ReplacerType.FilesAndFolderName
                     }" selected="selected">Files and folders</option>
...
const update = async (
  state: IState,
  set: (querySelector: string, html: string) => Promise<boolean>
) => {
  setStateRegExp(state)
  await getFoldersAndFiles(state)
  await set("#main", renderState(state))
}
...
const applyRenaming = async (state: IState) => {
  ...
  if (state.folders.length) {
    ...
    const sortedFolders = state.folders.sort(
...
const createRenameObject = (
  ...
  replacer: IReplacer,
  index: { value: number },
  prevList: IRenameObject[],
  toAddIndex?: boolean
): IRenameObject | undefined => {
  let newName = replacer.to
  let replaceHtml = ""
  let found = false
  let indexUsed = false
  if (
    (newName.includes("$index") && state.replaceIndex && !isFolder) ||
  ...
  if (state.useReg && replacer.fromRegExp) {
    ...
    if (match) {
      replaceHtml = replaceWithCaseOperations(
        oldName,
        replacer.fromRegExp,
        newName,
        (replaceText) =>
          `<span class='from'>${vsc.escapeHtml(
            match[0]
          )}</span><span class='to'>${vsc.escapeHtml(replaceText)}</span>`
      )
      let nextNewName = replaceWithCaseOperations(
        oldName,
        replacer.fromRegExp,
        newName
      )
  ...
  if (!state.useReg) {
    replaceHtml = oldName.replace(
      replacer.from,
      `<span class='from'>${vsc.escapeHtml(
        replacer.from
      )}</span><span class='to'>${vsc.escapeHtml(newName)}</span>`
    )
...
const getFoldersAndFiles = async (state: IState) => {
  ...
  const allFilePaths = await vsc.findFilePathsFromBase(
    state.selectedDir,
    includePath
  )
  ...
  if (state.replacer.appliesTo !== ReplacerType.FolderName) {
    allFilePaths.forEach((path) => {
      ...
      const renameObject = createRenameObject(
        dir,
        oldName,
        false,
        path,
...
const replaceWithCaseOperations = (
  text: string,
  regex: RegExp,
  simpleString: string,
  updateOut: (r: string) => string = (t) => t
): string => {
