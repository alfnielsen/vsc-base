...
export async function run(path: string) {
   ...
   const positions = vsc.tsFindAllNodePositionsFromContent(source, node => vsc.tsMatchImport(node))


   const imports = vsc.tsFindAllNodePositionsFromContent(source, node => vsc.tsMatchImport(node, {
      hasSpecifiersName: /Call/
   }))
   vsc.showMessage(`Found ${imports.length} imports`)
   vsc.showMessage(`Text: ${imports[0][0].getFullText()}`)
